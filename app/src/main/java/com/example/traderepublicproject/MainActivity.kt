...
class MainActivity : AppCompatActivity() {
    ...
    private fun setUpBtcPriceText(message: String?) {
        message?.let {
            ...
            val adapter: JsonAdapter<StockItemISIN> = moshi.adapter(StockItemISIN::class.java)
            val stock = adapter.fromJson(message)
            runOnUiThread { findViewById<TextView>(R.id.price_tv).text = "1 BTC: ${stock?.price} €" }
        }
    }
